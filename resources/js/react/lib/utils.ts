...
export const resolveModularPageComponent = (name: string): Promise<ResolvedComponent> => {
    if (name.includes('::')) {
        const [moduleName, componentPath] = name.split('::', 2);

        const moduleFolderName = moduleName.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
        const moduleGlobs = import.meta.glob<ResolvedComponent>('/modules/*/resources/js/**/pages/**/*.tsx');
